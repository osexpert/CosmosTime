...
[assembly: InternalsVisibleToAttribute("CosmosTime.UnitTests")]
...
namespace CosmosTime.TimeZone
{
	...
	public static class IanaTimeZone
	{
		...
		static readonly Lazy<Func<TimeZoneInfo, bool>> _hasIanaId = new(() =>
		{
			...
			if (prop != null)
			{
				return (Func<TimeZoneInfo, bool>)Delegate.CreateDelegate(typeof(Func<TimeZoneInfo, bool>), prop.GetGetMethod());
			}
			return null;
		});


		static readonly Lazy<bool> _tziSeemsToHaveIana = new(() =>
		{
			try
			{
				_ = TimeZoneInfo.FindSystemTimeZoneById("Etc/UTC");
				return true;
			}
		...
		static Func<S, T> CreateGetFieldDelegate<S, T>(this Type type, string fieldName)
		{
		...
		// Avoid a useless clone and also avoid a Linux bug in GetAdjustmentRules()
		...
		// This map is generated by a tool. Do not modify. 21.06.2023 10:03:05Z
		static readonly Dictionary<string, string> _ianaToWindows = new()
		{
		...
		// This map is generated by a tool. Do not modify. 21.06.2023 10:03:06Z
		static readonly Dictionary<string, string> _windowsToIana = new()
		{
		...
		public static string GetIanaId(string ianaId_or_windowsId)
		{
			if (TryGetIanaId(ianaId_or_windowsId, out var iana))
				return iana;

			throw new TimeZoneNotFoundException("Time zone not found: " + ianaId_or_windowsId);
		}
		...
		public static bool TryGetIanaId(string ianaId_or_windowsId, out string ianaId)
		{
			...
			// Also NET6+ now support it on Windows
			if (_ianaToWindows.ContainsKey(ianaId_or_windowsId))
			{
				ianaId = ianaId_or_windowsId;
				return true;
			}

			if (_windowsToIana.TryGetValue(ianaId_or_windowsId, out var iana))
			{
				ianaId = iana;
				return true;
			}
		...
		{
			if (TryGetWindowsId(ianaId, out var winowsId))
				return winowsId;
		...
		/// <returns></returns>
		public static bool TryGetWindowsId(string ianaId, out string windowsId) 
			=> _ianaToWindows.TryGetValue(ianaId, out windowsId);
		...
		static readonly ConcurrentDictionary<string, TimeZoneInfo> _customIanaTimeZones = new();
		...
		{
			// 1st, check if we have created some custom iana tz, in case we always want it first
			if (_customIanaTimeZones.TryGetValue(ianaId, out tz))
				return true;

			// then check if we are on net6 or later, that has HasIanaId property, then we don't need to use the builtin iana list
			if (_hasIanaId.Value != null)
			{
				try
				{
					tz = TimeZoneInfo.FindSystemTimeZoneById(ianaId);
					return _hasIanaId.Value(tz);
				}
				catch (TimeZoneNotFoundException)
				{ }
			}

			// finally use the builtin iana list
			if (TryGetWindowsId(ianaId, out var windowsId))
			{
				// in case some NET version before NET6 supported Iana?
				if (_tziSeemsToHaveIana.Value)
				{
					try
					{
						tz = TimeZoneInfo.FindSystemTimeZoneById(ianaId);
				...
				// lastly, lookup by windows tz and clone it into a custom iana tz
				try
				{
					var wtz = TimeZoneInfo.FindSystemTimeZoneById(windowsId);
					if (ianaId == windowsId) // || wtz == TimeZoneInfo.Utc) // strange with both, but maybe the Id is "utc", because Windows ids are insensitive...
					{
						tz = wtz;
					}
					else
					{
						var isUtc = IsUtc(ianaId);

						try
						{
							var custom_tz = TimeZoneInfo.CreateCustomTimeZone(ianaId,
								wtz.BaseUtcOffset,
								isUtc ? "(UTC) Coordinated Universal Time" : wtz.DisplayName,
								isUtc ? "Coordinated Universal Time" : wtz.StandardName,
								isUtc ? "Coordinated Universal Time" : wtz.DaylightName,
								_getRulesRaw.Value != null ? _getRulesRaw.Value(wtz) : wtz.GetAdjustmentRules());

							tz = _customIanaTimeZones.GetOrAdd(ianaId, custom_tz);
						}
						catch (InvalidTimeZoneException)
						{
							// it won't be the same name...but oh well
							tz = wtz;
						}
					}
					return true;
		...
		public static bool HasIanaId(TimeZoneInfo tz)
		{
			...
			if (_hasIanaId.Value != null)
				...
				return _ianaToWindows.ContainsKey(tz.Id);
		}
		...
		/// <param name="tz"></param>
		/// <returns></returns>
		public static bool IsUtc(TimeZoneInfo tz)
		{
